...
from shared.paths import STEP_04_LOSS_PNG, STEP_04_METRICS, STEP_04_SAMPLES_PNG
from shared.pokemon_dataset import build_loaders_from_manifest, get_metadata_from_png, metadata_to_conditioning, pick_device
...
@kfp_component(packages_to_install=["torch", "diffusers", "pillow", "matplotlib", "transformers"])
def evaluation_component(
    ...
    metrics_file = _write_json(metrics_output.path, metrics)
    ...
    unet.load_state_dict(torch.load(checkpoint_path.path, map_location=device, weights_only=False)["unet_state_dict"])
    unet.eval()
    ...
    for column, index in enumerate(indices):
        path = get_val_path(index)
        conditioning = metadata_to_conditioning(get_metadata_from_png(path))
        axes[0, column].imshow(
            pipe(conditioning, num_inference_steps=num_inference_steps, guidance_scale=guidance_scale).images[0]
        )
        axes[0, column].axis("off")
        axes[1, column].imshow(Image.open(path).convert("RGB").resize((512, 512)))
